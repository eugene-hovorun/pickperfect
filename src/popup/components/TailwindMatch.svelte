<script lang="ts">
  ...
  let accuracy = $derived(Math.max(0, 100 - (match.distance / 441.67) * 100));
  let textColor = $derived(luminance(color) > 0.6 ? "#1D1D1F" : "#FFFFFF");
  let matchTextColor = $derived(
    luminance(match.hex) > 0.6 ? "#1D1D1F" : "#FFFFFF",
  );
</script>
...
<section class="bg-muted/50 rounded-xl p-3 flex flex-col gap-3">
  <div class="flex items-center justify-between">
    <span class="text-[11px] font-medium text-muted-foreground tracking-wider">
      {t("nearestTailwindTitle")}
    </span>
    {#if isExactMatch}
      <span
        class="text-[10px] font-semibold px-1.5 py-0.5 bg-green-500/15 text-green-600 dark:text-green-500 rounded tracking-wide"
      >
        {t("exactMatch")}
      </span>
    {:else}
      <span class="text-[11px] font-medium text-muted-foreground">
        {t("percentMatch", accuracy.toFixed(0))}
      </span>
    {/if}
  </div>
